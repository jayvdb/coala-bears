...
if [ "$CIRCLE_BUILD_IMAGE" = "ubuntu-12.04" ]; then
  ...
  deps_go="golang"
  deps_specific_versions=""
else
  # Use xenial, needed to replace outdated julia on the Circle CI trusty image
  echo "deb http://archive.ubuntu.com/ubuntu/ xenial main universe" | sudo tee -a /etc/apt/sources.list.d/xenial.list > /dev/null
  # However the go provided by Circle CI on the trusty image is acceptable
  deps_go=""
...
deps="espeak libclang1-3.4 indent mono-mcs chktex hlint r-base julia luarocks verilator cppcheck flawfinder libxml2-utils"
